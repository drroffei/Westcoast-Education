...
function CoursesDelete() {
  ...
  const onDeleteClickHandler = async () => {
    ...
    const url = `${process.env.REACT_APP_BASEURL}/course/${params.id}`;
    const response = await fetch(url, {
      method: 'DELETE',
    });
    ...
    if (response.status >= 200 && response.status <= 299) {
      alert('Kursen togs bort från databasen')
    }
    else {
      alert('VARNING: Kursen togs INTE bort');
    }
  };
  ...
  return (
    <>
      <div className="page-section">
        ...
        <h3>Här kan du ta bort följande kurs från databasen:</h3>
        <div className="page-content">
          <form className="form" onSubmit={handleSaveCourse}>
            ...
            <div className="button">
              <p>Bekräfta borttagning genom att klicka på: <span onClick={onDeleteClickHandler}>
